...
\begin{figure}[ht] % ’ht’ tells LaTeX to place the figure ’here’ or at the top of the page
  ...
  \begin{tikzpicture}[shorten >=1pt,node distance=3cm,on grid,auto]
   ...
   \draw[->] (Init) edge [bend right] node {\tt -LAU} (LAUSnd);
   \draw[->] (Init) edge [bend left] node {\tt +LAU} (LAURecv);
   \draw[->] (LAUSnd) edge [bend right] node {\tt +LAU} (LAUSync);
   \draw[->] (LAURecv) edge [bend left] node {\tt -LAU} (LAUSync);
   \draw[->] (LAUSync) edge [bend right] node {\tt -Br} (BrSnd);
   \draw[->] (LAUSync) edge [bend left] node {\tt +Br} (BrRecv);
   \draw[->] (LAUSync) edge [loop left] node {\tt LAUSuccess} ();
   \draw[->] (LAUSync) edge [bend right] node {\tt LAUFail} (Init);
   \draw[->] (BrSnd) edge [bend right] node {\tt +Br} (BrSync);
   \draw[->] (BrRecv) edge [bend left] node {\tt -Br} (BrSync);
